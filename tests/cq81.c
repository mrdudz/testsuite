...
#define OUTFILE  "cq81.out"
#define THISFILE "cq81"
...
/*    The storage class specifiers are:
   ...
   if ( crc != 0 ) {
     rc = rc+1;
     if( pd0 -> flgd != 0 ) fprintf(outfile,s81er,1);
   }

   if ( prc != 0 ) {
     rc = rc+2;
     if( pd0 -> flgd != 0 ) fprintf(outfile,s81er,2);
   }

   if ( irc != 0 ) {
     rc = rc+4;
     if( pd0 -> flgd != 0 ) fprintf(outfile,s81er,4);
   }
...
/*   Now we check to see if variables are actually being assigned
   ...
   k = regc();
   if ( pd0->flgm != 0 ) {
     if ( k < 0 ) fprintf(outfile,badtest,"char");
     else fprintf(outfile,goodtest,k,"char");
   }

   k = regp();
   if ( pd0->flgm != 0 ) {
     if ( k<0 ) fprintf(outfile,badtest,"pointer");
     else fprintf(outfile,goodtest,k,"pointer");
   }

   k = regi();
   if ( pd0->flgm != 0 ) {
     if ( k<0 ) fprintf(outfile,badtest,"int");
     else fprintf(outfile,goodtest,k,"int");
   }
