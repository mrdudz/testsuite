...
#define OUTFILE  "cq88.out"
#define THISFILE "cq88"
...
int s88(struct defs *pd0){
   ...
   if(sizeof distance != sizeof(int)){
     if(pd0->flgd != 0) fprintf(outfile,s88er,1);
     rc = rc+1;
   }
   ...
   *metricp = 3;
   ...
   if(distance != 3){
     if(pd0->flgd != 0) fprintf(outfile,s88er,2);
     rc = rc+2;
   }
   ...
   if(z.re+z.im != 2){
   #endif
     if(pd0->flgd != 0) fprintf(outfile,s88er,4);
     rc = rc+4;
   }
