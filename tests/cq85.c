...
#define OUTFILE  "cq85.out"
#define THISFILE "cq85"
...
int s85(struct defs *pd0){
   ...
   if( (char *)&s1.count - &s1.tword[0] <= 0
     ...
     if(pd0->flgd != 0) fprintf(outfile,s85er,1);
     rc = rc+1;
   }
   ...
   if(s3.threebit != 3){
     if(s3.threebit == -1){
       if(pd0->flgm != 0) fprintf(outfile,"Sign extension in fields\n");
     }
     else{
   	   #ifdef NO_BITFIELDS
       		if(pd0->flgd != 0) fprintf(outfile,"NO_BITFIELDS\n");
	   #else
       		if(pd0->flgd != 0) fprintf(outfile,s85er,2);
       		rc = rc+2;
	   #endif
     }
   }
   ...
   if(s3.onebit != 1){
     if(pd0->flgm != 0)
      fprintf(outfile,"Be especially careful with 1-bit fields!\n");
   }
   ...
   if( (char *)u0.u1 - (char *)&u0 != 0
     ...
     if(pd0->flgd != 0) fprintf(outfile,s85er,4);
     rc = rc+4;
   }
   ...
   if( sizeof u0 < sizeof u0.u1
     ...
     if(pd0->flgd != 0) fprintf(outfile,s85er,8);
     rc = rc+8;
   }
   ...
   s1.right->tword[0] += 1;
   if(s2.tword[0] != 3){
     if(pd0->flgd != 0) fprintf(outfile,s85er,16);
     rc = rc+16;
   }
   return rc;
}
