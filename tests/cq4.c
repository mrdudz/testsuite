...
the block.                                                     */
   ...
   for (j=0; j<3; j++)
     if(svtest(j) != zero()){
       rc = 1;
       if(pd0->flgd != 0) fprintf(outfile,s4er,1);
     }
   ;
...
unication between functions, even separately compiled functions.
   ...
   if(testev() != 0){
     rc=rc+2;
     if(pd0->flgd != 0) fprintf(outfile,s4er,2);
   }
/*
...
integers.
   ...
   if(sizeof lint < sizeof pint || sizeof pint < sizeof sint){
     ...
     rc = rc+4;
     if(pd0->flgd != 0) fprintf(outfile,s4er,4);
   }
...
implementation                                                  */
   ...
   if(mask != 1 || target != 0){
     ...
     rc = rc+8;
     if(pd0->flgd != 0) fprintf(outfile,s4er,8);
   }
