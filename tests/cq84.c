...
#define OUTFILE  "cq84.out"
#define THISFILE "cq84"
...
*/
   ...
   ip = fip(3);
   if(*ip != 3){
     if(pd0->flgd != 0) fprintf(outfile,s84er,1);
     rc = rc+1;
   }
   ...
   if(glork(4) != 4){
     if(pd0->flgd != 0) fprintf(outfile,s84er,2);
     rc = rc+2;
   }
	#else
   pfi = glork;
   ...
   if(sum != 136){
     if(pd0->flgd != 0) fprintf(outfile,s84er,4);
     rc = rc+4;
   }
   ...
   if( array(x3d,105,0)
      ...
      if(pd0->flgd != 0) fprintf(outfile,s84er,8);
      rc = rc+8;
   }
