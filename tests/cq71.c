...
#define OUTFILE  "cq71.out"
#define THISFILE "cq71"
...
/*   A string is a primary expression. The identifier points to
   ...
   if(*"queep" != q){
     rc = rc+1;
     if(pd0->flgd  != 0) fprintf(outfile,s71er,1);
   }
/*   A parenthesized expression is a primary expression whose
   ...
   if((2+3) != 2+3) {
     rc = rc+2;
     if(pd0->flgd != 0) fprintf(outfile,s71er,2);
   }
...
/*   A primary expression followed by an expression in square
   ...
   if(x[5] != 1942 || x[5] != *((x)+(5))){
     rc = rc+4;
     if(pd0->flgd != 0) fprintf(outfile,s71er,4);
   }
...
/*   If the various flavors of function calls didn't work, we
   ...
   if ( McCarthy(-5) != 91){
     rc = rc+8;
     if(pd0->flgd != 0) fprintf(outfile,s71er,8);
   }
...
/*   and that argument passing is strictly by value.           */
   ...
   if(a != 2 || b != 2){
     rc = rc+16;
     if(pd0->flgd != 0) fprintf(outfile,s71er,16);
   }
...
/*   Finally, structures and unions are addressed thusly:      */
   ...
   if(pd0->dprec != (*pd0).dprec){
     rc = rc+32;
     if(pd0->flgd != 0) fprintf(outfile,s71er,32);
   }
