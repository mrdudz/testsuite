...
#define OUTFILE  "cq7813.out"
#define THISFILE "cq7813"
...
int s7813(struct defs *pd0){
   ...
   if(((char)12 & (char)10) !=  8) {lrc = 1;
      if(prlc) fprintf(outfile,fl,lrc);}
   ...
   if(((int)12 ^ (unsigned)10) !=  6) {lrc = 72;
      if(prlc) fprintf(outfile,fl,lrc);}
   if(((int)12 & (int)10) !=  8) {lrc = 73; if(prlc) fprintf(outfile,fl,lrc);}
   if(((int)12 | (int)10) != 14) {lrc = 74; if(prlc) fprintf(outfile,fl,lrc);}
   if(((int)12 ^ (int)10) !=  6) {lrc = 75; if(prlc) fprintf(outfile,fl,lrc);}

   if(lrc != 0){
     if(pd0->flgd != 0) fprintf(outfile,s7813er,1);
     rc = rc+1;
   }
   ...
   r = i++ && j++;
    if(i!=1) {lrc = 1; if(prlc) fprintf(outfile,fl,lrc);}
    if(j!=0) {lrc = 2; if(prlc) fprintf(outfile,fl,lrc);}
    if(r!=0) {lrc = 3; if(prlc) fprintf(outfile,fl,lrc);}
   r = i && j++;
    if(i!=1) {lrc = 4; if(prlc) fprintf(outfile,fl,lrc);}
    if(j!=1) {lrc = 5; if(prlc) fprintf(outfile,fl,lrc);}
    if(r!=0) {lrc = 6; if(prlc) fprintf(outfile,fl,lrc);}
   r = i-- && j;
    if(i!=0) {lrc = 7; if(prlc) fprintf(outfile,fl,lrc);}
    if(j!=1) {lrc = 8; if(prlc) fprintf(outfile,fl,lrc);}
    if(r!=1) {lrc = 9; if(prlc) fprintf(outfile,fl,lrc);}
   r = i && j--;
    if(i!=0) {lrc = 10; if(prlc) fprintf(outfile,fl,lrc);}
    if(j!=1) {lrc = 11; if(prlc) fprintf(outfile,fl,lrc);}
    if(r!=0) {lrc = 12; if(prlc) fprintf(outfile,fl,lrc);}

   if(lrc!=0){
     if(pd0->flgd != 0) fprintf(outfile,s7813er,2);
     rc = rc+2;
   }
   ...
   r = j++ || i;
    if(i!=1) {lrc = 4; if(prlc) fprintf(outfile,fl,lrc);}
    if(j!=1) {lrc = 5; if(prlc) fprintf(outfile,fl,lrc);}
    if(r!=1) {lrc = 6; if(prlc) fprintf(outfile,fl,lrc);}
   r = i-- || j--;
    if(i!=0) {lrc = 7; if(prlc) fprintf(outfile,fl,lrc);}
    if(j!=1) {lrc = 8; if(prlc) fprintf(outfile,fl,lrc);}
    if(r!=1) {lrc = 9; if(prlc) fprintf(outfile,fl,lrc);}
   r = i || j--;
    if(i!=0) {lrc = 10; if(prlc) fprintf(outfile,fl,lrc);}
    if(j!=0) {lrc = 11; if(prlc) fprintf(outfile,fl,lrc);}
    if(r!=1) {lrc = 12; if(prlc) fprintf(outfile,fl,lrc);}

   if(lrc!=0){
     if(pd0->flgd != 0) fprintf(outfile,s7813er,4);
     rc = rc+4;
   }
   ...
   r = one?zero:one?i++:j++;
   if(r!=0 || i!=0 || j!=0){
     if(pd0->flgd != 0) fprintf(outfile,s7813er,8);
     rc = rc+8;
   }
   ...
   if((one?zero:1) != 0 || (zero?1:zero) != 0){
     if(pd0->flgd != 0) fprintf(outfile,s7813er,16);
     rc = rc+16;
   }
   return rc;
}
