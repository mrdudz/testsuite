...
#define OUTFILE  "cq86.out"
#define THISFILE "cq86"
...
int s86(struct defs *pd0){
   ...
   if(*pint != 5){
     if(pd0->flgd != 0) fprintf(outfile,s86er,1);
     rc = rc+1;
   }
   ...
   if(*apint != 3){
     if(pd0->flgd != 0) fprintf(outfile,s86er,2);
     rc = rc+2;
   }

   if(*rpint != 5){
     if(pd0->flgd != 0) fprintf(outfile,s86er,4);
     rc = rc+4;
   }
   ...
   for(j=0; j<10; j++)
     if(zero[j] != 0) lrc = 1;
   if(lrc != 0){
     if(pd0->flgd != 0) fprintf(outfile,s86er,8);
     rc = rc+8;
   }
   ...
   if(lrc != 0){
     if(pd0->flgd != 0) fprintf(outfile,s86er,16);
     rc = rc+16;
   }
   ...
   if(lrc != 0){
     if(pd0->flgd != 0) fprintf(outfile,s86er,32);
     rc = rc+32;
   }
   return rc;
}
