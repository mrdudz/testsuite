...
#ifdef DIJKSTRA_PRINTDEBUG
    if(node!=NULL) fprintf(outfile,"next cheapest Node: >%c<\n",node->TAG);
#endif
...
DIJKSTRA_NODE *currnode,*nextnode;
    ...
    fprintf(outfile,"Path from >%c< to >%c< : ",end->TAG,graph->TAG);

    currnode=end;

        while(currnode->PREVIOUS!=NULL){
            fprintf(outfile,">%c< ",currnode->TAG);
