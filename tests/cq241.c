...
#define OUTFILE  "cq241.out"
#define THISFILE "cq241"
...
/*   long d[39], o[39], x[39]; */
   ...
   if (   8 !=  010
     ...
     rc = rc+1;
     if( pd0->flgd != 0 ) fprintf(outfile,s241er,1);
   }
   ...
   if ( 0x00abcdef != 0xabcdef
     ...
     rc = rc+2;
     if( pd0->flgd != 0 ) fprintf(outfile,s241er,2);
   }
...
/*#warning "sizeof(long)!=4, skipping test"*/
   ...
   if ( sizeof 010000000000 != sizeof(long)      /* 2**30 */
     ...
     rc = rc+4;
     if( pd0->flgd != 0 ) fprintf(outfile,s241er,4);
   }
#endif
   ...
   if ( sizeof   67l != sizeof(long)
     ...
     rc = rc+8;
     if( pd0 -> flgd != 0 ) fprintf(outfile,s241er,8);
   }
   ...
   for (j=0; j<39; j++){
     if ( g[j] != d[j]
       ...
       if( pd0 -> flgm != 0 ) {
/*       fprintf(outfile,s241er,16);          save in case opinions change...     */
         fprintf(outfile,"Decimal and octal/hex constants sometimes give\n");
         fprintf(outfile,"   different results when assigned to longs.\n");
       }
/*     lrc = 1;   save...   */
     }
   }
