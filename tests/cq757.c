...
#define OUTFILE  "cq757.out"
#define THISFILE "cq757"
...
s26(struct defs *pd0) {
   ...
   if(pd0->flgm != 0) {
     fprintf(outfile,s,pd0->cbits,"char");
     fprintf(outfile,s,pd0->ibits,"int");
     fprintf(outfile,s,pd0->sbits,"short");
     fprintf(outfile,s,pd0->lbits,"long");
     fprintf(outfile,s,pd0->ubits,"unsigned");
     fprintf(outfile,s,pd0->fbits,"float");
     fprintf(outfile,s,pd0->dbits,"double");
     #ifndef NO_FLOATS
...
int s757(struct defs *pd0){
   ...
   if(t<<3<<2 != 1280 || t>>3>>2 != 1){
     rc = rc+1;
     if(pd0->flgd != 0) fprintf(outfile,s757er,1);
   }
   ...
   if(lrc != 0){
     rc = rc+2;
     if(pd0->flgd != 0) fprintf(outfile,s757er,2);
   }
   ...
   if((a<b<c) != 1){
     rc = rc+4;
     if(pd0->flgd != 0) fprintf(outfile,s757er,4);
   }
   ...
   if( &x[1] == &x[0] ){
     rc = rc+8;
     if(pd0->flgd != 0) fprintf(outfile,s757er,8);
   }

   if( &x[1] < &x[0] ) if(pd0->flgm != 0)
     fprintf(outfile,"Increasing array elements assigned to decreasing locations\n");
   ...
   if(lrc != 0){
     rc = rc+16;
     if(pd0->flgd != 0) fprintf(outfile,s757er,16);
   }
   ...
   if(p != 0){
     rc = rc+32;
     if(pd0->flgd != 0) fprintf(outfile,s757er,32);
   }
