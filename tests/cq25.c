...
#define OUTFILE  "cq25.out"
#define THISFILE "cq25"
...
int s25(struct defs *pd0) {
   ...
   if ( s[0] != s[1] || s[1] != s[2]
    ...
    rc = rc+1;
     if(pd0->flgd != 0) fprintf(outfile,s25er,1);
   }
   ...
   if( s[3] != '\0' ){
     rc = rc+4;
     if(pd0->flgd != 0) fprintf(outfile,s25er,4);
   }
   ...
   if( ".\"."[1] != '"' ){
    rc = rc+8;
     if(pd0->flgd != 0) fprintf(outfile,s25er,8);
   }
   ...
   if( s[0] != '\n'
    ...
    || s[6] != '\'' ){
     ...
     rc = rc+16;
     if( pd0->flgd != 0) fprintf(outfile,s25er,16);
   }
   ...
   if (lrc != 0){
     rc = rc+32;
     if(pd0->flgd != 0) fprintf(outfile,s25er,32);
   }
   return rc;
}
