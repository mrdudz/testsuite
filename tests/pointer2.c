...
#define OUTFILE  "pointer2.out"
#define THISFILE "pointer2"
...
int a;
	...
	a=sizeof(i1[0]);
	fprintf(outfile,"%04x - ",a);
	if(sizeof(i1[0])==sizeof(char)) {
		/* gcc gives size of element */
		fprintf(outfile,"sizeof(i1[0]) gives size of element\n");
	}
	if(sizeof(i1[0])==sizeof(char*)) {
		fprintf(outfile,"sizeof(i1[0]) gives size of pointer to element\n");
	}
}
...
/*		fprintf(outfile,"%02x %02x %02x %02x\n",c1,c2,c3,c4); */
	...
	if(e) fprintf(outfile,"test2 failed.\n");
	else fprintf(outfile,"test2 ok.\n");
}
