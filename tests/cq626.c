...
#define OUTFILE  "cq626.out"
#define THISFILE "cq626"
...
int s626(struct defs *pd0){
   ...
   if(f1>2.*pd0->fprec){
     ...
     rc = rc+2;
     if(pd0->flgd != 0) fprintf(outfile,s626er,2);
   }
   ...
   if(t0 != 0){
     ...
     if(pd0->flgd != 0){
       ...
       fprintf(outfile,s626er,4);
       fprintf(outfile,"   key=");
       for(j=0;j<28;j++) fprintf(outfile,"%d",t[j]);
       fprintf(outfile,"\n");
     }
   }
   ...
   if((long)l > (unsigned)0100000){
      ...
      rc = rc+8;
      if(pd0->flgd != 0) fprintf(outfile,s626er,8);
   }
