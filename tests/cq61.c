...
#define OUTFILE  "cq61.out"
#define THISFILE "cq61"
...
involves sign extension.                                */
   ...
   if(to != -19){
     rc = rc+1;
     if(pd0->flgd != 0) fprintf(outfile,s61er,1);
   }
   else if(pd0->flgd != 0) fprintf(outfile,s61ok,1);
...
acter set is nonnegative.                               */
   ...
   if(lrc != 0){
     rc=rc+2;
     if(pd0->flgd != 0) fprintf(outfile,s61er,2);
   }
   else if(pd0->flgd != 0) fprintf(outfile,s61ok,2);
...
simply discarded.                                       */
   ...
   if((shortint != longint && shortint != 3) ||
     ...
     rc = rc+8;
     if(pd0->flgd != 0) fprintf(outfile,s61er,8);
   }
   else if(pd0->flgd != 0) fprintf(outfile,s61ok,8);
